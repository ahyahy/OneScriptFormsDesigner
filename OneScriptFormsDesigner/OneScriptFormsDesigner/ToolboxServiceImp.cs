...
namespace osfDesigner
{
    ...
    // Конструкторы постоянно запрашивают панель элементов, когда
    // курсор над ней, чтобы получить обратную связь с выбранным элементом управления
    // НАПОМИНАНИЕ:
    //     Этот класс реализует интерфейс IToolboxService
    //     он НЕ создает ПолеСписка (ListBox), он просто связывает один
    //     созданный пользователем элемент, а затем на него ссылается через
    //     ToolboxServiceImp::Toolbox свойство
    ...
    public class ToolboxServiceImp : IToolboxService
    {
        ...
        public ToolboxServiceImp( IDesignerHost host )
        {
            ...
            // Наша MainForm добавляет нашу панель элементов (ToolboxPane) к услугам DesignerHost.
            Toolbox = null;
		}
        ...
        void IToolboxService.AddCreator( ToolboxItemCreatorCallback creator, string format, IDesignerHost host )
        {
            ...
            //через new NotImplementedException();
        }

        // Добавление создателя, который преобразует нестандартные элементы в указанном формате в элементы панели 
        // элементов (ToolboxItems).
        ...
        // Добавление элемента (ToolboxItem) к нашей панели элементов, в определенной категории, связанного с определенным 
        // узлом.
        ...
        // Наша панель элементов имеет категории, похожие на категории Visual Studio, но вы
        // можете группировать их любым способом. Просто поставьте вашу службу IToolboxService в известность.
        // что категорий нет, верните Неопределено
        CategoryNameCollection IToolboxService.CategoryNames
        {
            get { return null; }
        }

        // Необходимо для перетаскивания
        ...
        // Мы всегда используем стандартные элементы (ToolboxItems), поэтому они всегда поддерживаются
        // Поэтому возвращаем Истина
        bool IToolboxService.IsSupported( object serializedObject, ICollection filterAttributes )
        {
            return true;
        }

        // Мы всегда используем стандартные элементы, поэтому они всегда поддерживаются. 
        // Поэтому возвращаем Истина
        ...
        // В нашем случае все наши элементы являются стандартными и из набора констант, и все они элементы 
        // панели элементов, так что если мы можем десериализовать его в нашем стандартном пути, тогда 
        // это действительно элемент панели элементов (ToolboxItem)
        // Узел игнорируется
        ...
        // В нашем случае все наши элементы являются стандартными и из набора констант, и все они элементы 
        // панели элементов, так что если мы можем десериализовать его в нашем стандартном пути, тогда 
        // это действительно элементы панели элементов (ToolboxItem)
        ...
        // Этот метод вызывается после вызова метода ToolPicked.
        // В нашем случае мы выбираем указатель. 
        ...
        // Сериализация элемента, необходимая для перетаскивания
        // Мы сериализуем панель элементов, упаковывая ее в объект данных (DataObject)
        ...
        // Если выбран инструмент, мы возможно, захотим установить курсор в какое нибудь
        // интересующее место над областью конструктора. Если мы это сделаем, то
        // мы определим это здесь и вернем Истина. В противном случае мы возвращаем значение Ложь, чтобы вызывающий 
